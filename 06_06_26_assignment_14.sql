...
SELECT COUNT(DISTINCT node_id) AS No_of_Nodes FROM customer_nodes;

-- What is the number of nodes per region?
SELECT r.region_name, COUNT(DISTINCT c.node_id) AS No_of_Customer
FROM customer_nodes c INNER JOIN regions r
ON c.region_id = r.region_id
GROUP BY r.region_name;

-- How many customers are allocated to each region?
SELECT r.region_name, COUNT(DISTINCT c.customer_id) AS No_of_Customer
FROM customer_nodes c INNER JOIN regions r
ON c.region_id = r.region_id
GROUP BY r.region_name;

-- How many days on average are customers reallocated to a different node?
SELECT AVG(DATEDIFF(day, start_date, end_date)) AS avg_days -- with help od DATEDIFF() Fuction
FROM customer_nodes
...
SELECT DISTINCT r.region_name,
    ...
    OVER (PARTITION BY c.region_id) AS median_days,

    PERCENTILE_CONT(0.8)
    WITHIN GROUP (ORDER BY DATEDIFF(day, c.start_date, c.end_date))
    OVER (PARTITION BY c.region_id) AS p80_days,

    PERCENTILE_CONT(0.95)
    WITHIN GROUP (ORDER BY DATEDIFF(day, c.start_date, c.end_date))
    OVER (PARTITION BY c.region_id) AS p95_days
...
(SELECT COUNT(txn_type) AS deposit_count, SUM(txn_amount) AS deposit_amount
FROM customer_transactions
